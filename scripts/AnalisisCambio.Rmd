...
El análisis de cambio se hizo con las capas de entrada de Uso de Suelo y Vegetación del SMAOT de los años 2009, 2018 y 2020 donde se usó el ID de "tipo" del año 2009 para unificar y hacer el análisis comparativo. Las capas se encuentran en Sistema de coordenadas WGS 1984 UTM Zona 14 en metros y Datum WGS 1984. La capa original (shapefile) se transformó a formato raster en ArcMap v. 10.4.1 con la función "Polygon to raster" usando un tamaño de pixel es de 100 m.
...
## Procesando Insumos: Uso de Suelo y Vegetación (Estado)
...
#NOTAS IMPORTANTES
Los raster llamados usvedo_año_reclas.tif provienen de la conversión de shapefile a raster y la reclasificación para unificar las categorías. ADEMÁS se dejaron los mismos cuerpos de agua del año 2020 
...
Actualización(03/Sep/2024): Se actualizan los insumos para que coincidan en extensión. Usar los "mapa_2009_ext.tif" y "mapa_2009_ext.tif"
...
#No usar porque no tiene los mismos cuerpos de agua que usv20
#edo09 <- terra::rast("D:/mapas_gto/UTM/conectividad_estructural/inputs_corregidos/usv_rasterizados/usvedo_2009_reclas.tif")
...
#COLUMNAS DE ID PARA LOS TIPOS DE VEGETACIÓN CATEGÓRICOS 
...
#PREPARACIÓN DE INSUMOS(RECORTES): Procesando recortes de COBIOCOM y ANP's a partir de las capas de usv
Nota: Esta parte del código solo se corre si no se tienen las capas en formato raster recortadas a las diferentes escalas, en caso de tenerlas disponibles con crs EPSG:32614 (UTM Zona 14) y en metros entonces solo leer las capas y pasar al siguiente paso (generar las métricas).
```{r}
...
#Leer todos los insumos de formato .tif
...
#obteniendo los mapas en loop y exportando en formato raster 
...
##NO CORRER SI YA SE CORRIÓ EL LOOP ANTERIOR
# Loop para ahorra espacio en memoria 
for(i in 1:length(finales_parche)){
...
recortes_20
 ...
 ##dataframe con el tipo de cubierta que corresponde a cada raster. Nota: ya no es necesario porque 
#están unificadas las clases
#id_edo09 <- levels(edo09) %>% as.data.frame() %>% mutate(class = value) %>% select(class,tipo)
#id_edo20 <- levels(edo20) %>% as.data.frame() %>% mutate(class = value) %>% mutate(tipo=Tipo) %>% select(class,tipo)
